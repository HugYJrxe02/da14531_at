...
static const uint8_t CUST1_CP_TX_UUID_128[ATT_UUID_128_LEN]       = DEF_CUST1_CP_TX_UUID_128;
static const uint8_t CUST1_CP_RX_UUID_128[ATT_UUID_128_LEN]        = DEF_CUST1_CP_RX_UUID_128;

static struct att_char128_desc custs1_server_rx_char        = {ATT_CHAR_PROP_WR_NO_RESP,
                                                              {0, 0},
                                                              DEF_CUST1_CP_RX_UUID_128};

static struct att_char128_desc custs1_server_tx_char        = {ATT_CHAR_PROP_NTF,
                                                              {0, 0},
                                                              DEF_CUST1_CP_TX_UUID_128};
...
const struct attm_desc_128 custs1_att_db[CUSTS1_IDX_NB] =
{
    ...
    [CUST1_IDX_CP_RX_CHAR]          = {(uint8_t*)&att_decl_char, ATT_UUID_16_LEN, PERM(RD, ENABLE),
                                            sizeof(custs1_server_rx_char), sizeof(custs1_server_rx_char), (uint8_t*)&custs1_server_rx_char},

    // Server RX Characteristic Value
    [CUST1_IDX_CP_RX_VAL]           = {CUST1_CP_RX_UUID_128, ATT_UUID_128_LEN, PERM(WR, ENABLE) | PERM(WRITE_COMMAND, ENABLE),
                                            DEF_CUST1_CP_RX_CHAR_LEN, 0, NULL},

    // Server RX Characteristic User Description
    [CUST1_IDX_CP_RX_USER_DESC]     = {(uint8_t*)&att_desc_user_desc, ATT_UUID_16_LEN, PERM(RD, ENABLE),
                                            sizeof(CUST1_CP_RX_USER_DESC) - 1, sizeof(CUST1_CP_RX_USER_DESC) - 1, CUST1_CP_RX_USER_DESC},

    // Server TX Characteristic Declaration
    [CUST1_IDX_CP_TX_CHAR]          = {(uint8_t*)&att_decl_char, ATT_UUID_16_LEN, PERM(RD, ENABLE),
                                            sizeof(custs1_server_tx_char), sizeof(custs1_server_tx_char), (uint8_t*)&custs1_server_tx_char},

    // Server TX Characteristic Value
    [CUST1_IDX_CP_TX_VAL]           = {CUST1_CP_TX_UUID_128, ATT_UUID_128_LEN, PERM(NTF, ENABLE)|PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE),
                                            DEF_CUST1_CP_TX_CHAR_LEN, 0, NULL},

    // Server TX Client Characteristic Configuration Descriptor
    [CUST1_IDX_CP_TX_NTF_CFG]       = {(uint8_t*)&att_desc_cfg, ATT_UUID_16_LEN, PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE),
                                            sizeof(uint16_t), 0, NULL},

    // Server TX Characteristic User Description
    [CUST1_IDX_CP_TX_USER_DESC]     = {(uint8_t*)&att_desc_user_desc, ATT_UUID_16_LEN, PERM(RD, ENABLE),
                                            sizeof(CUST1_CP_TX_USER_DESC) - 1, sizeof(CUST1_CP_TX_USER_DESC) - 1, CUST1_CP_TX_USER_DESC},
